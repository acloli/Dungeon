...
- [x] **UIの責務分割:** `BattleSceneController` などを整理し、View / Presenter パターンへのリファクタリング（TFramework の Page フロー移行）
- [x] **正式なデータパイプライン:** CSV/Excel等を用いたマスターデータ自動生成ワークフローを導入し、Battle runtime の参照先を MasterData ベースへ移行
- [ ] **リソース管理の最適化:** Addressables を本格運用し、プレハブやアセットの非同期ロード・メモリ管理を徹底
- [x] **UIUXの改善:** Battle表示層専用UIへの移行、およびターゲット先行操作による誤操作防止UXの導入
...
- [x] Main / Title / Run 入口側の RunProfile 指定導線を整理
- [x] 状態異常・Intent 表示・複数対象処理などの Battle 仕様を MasterData 定義へ追従
...
（完了後の次候補として以下の実装を優先する）

- [x] **RunSaveによるゲーム内状態保存:** 進行状況の保存と再開機能の実装
- [ ] **マップイベントと報酬:** RestShopの実機能拡張、Card reward以外の報酬追加、Map event nodeの実装
- [ ] **パッシブ効果システム:** レリック（アーティファクト）等の永続効果を管理・フックするシステムの構築
- [ ] **高度なカードエフェクト:** バフ・デバフ（状態異常）システムの構築と、エフェクトチェーン（EffectChain）の拡張
- [ ] **エネミーAIの拡張:** より複雑な行動パターン、および条件分岐の実装
